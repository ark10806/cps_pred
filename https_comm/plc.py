...
with sock.socket(sock.AF_INET, sock.SOCK_STREAM) as clnt:
	...
	time.sleep(1)
	data = {'fur_ID': fur_ID, 'mat_ID': 0, 'proc_ID': 0, 'amount': 100, 'feedback': 'null', 'span': '100'}
	res = requests.post(url+'/start', data=json.dumps(data), headers=headers, verify=False)
	print(res.status_code)
	#time.sleep(5)
	while True:
		data = {'fur_ID': fur_ID, 'temps': [{'temp': 1}, {'temp': 2}, {'temp': 3}, {'temp': 4}, {'temp': 5}, {'temp': 6}] , 'press': f'{press}', 'flow': 0, 'is_closed': 'true'}
