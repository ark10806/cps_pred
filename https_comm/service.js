...
class Furnaces{
	...
	async start(fur_ID, mat_ID, proc_ID, amount, feedback, span){
		...
		exec_sql(sql).then((value) => {
			...
			console.log(`Furnace start with operation_No [${operation_cycle}]`);
		});
	}
	async insert_values(fur_ID, t, press, flow, is_closed){
		...
		const sql = `INSERT INTO archive_fur${fur_ID} VALUES(0, ${timestamp.now()/1000}, ${t[0]}, ${t[1]}, ${t[2]}, ${t[3]}, ${t[4]}, ${t[5]}, ${press}, ${flow}, ${is_closed})`
		exec_sql(sql).then((value) => {
