...
query = 'CREATE TABLE archive_fur%s(operation_No int not null, relative_time int not null, timestamp int not null, is_closed boolean not null, temp1 int(4) not null, temp2 int(4) not null, temp3 int(4) not null, temp4 int(4) not null, temp5 int(4) not null, temp6 int(4) not null, flow int(4) not null,  press int(4) not null, PRIMARY KEY(operation_No, relative_time))'
for i in range(total_furnace):
	cur.execute(query, i+1)
...
for i in range(total_furnace):
	cur.execute(query, i+1)
query = 'CREATE TABLE curr_fur%s(relative_time INT(4) NOT NULL AUTO_INCREMENT PRIMARY KEY, timestamp INT NOT NULL, is_closed BOOLEAN NOT NULL, temp1 INT(4) NOT NULL, temp2 INT(4) NOT NULL, temp3 INT(4) NOT NULL, temp4 INT(4) NOT NULL, temp5 INT(4) NOT NULL, temp6 INT(4) NOT NULL, flow INT(4) NOT NULL, press INT(4) NOT NULL)'
for i in range(total_furnace):
	cur.execute(query, i+1)
